...
public class AppUserBean {
    ...
    public boolean loginUserJSPM2(String username, String password) {
        System.out.println("loginUserJSPM2 start... " +username + " " + password);
        Connection connection = null;
        ResultSet rs = null;
        Object values[] = {username};
        try {
            ...
            rs = pstmt.executeQuery();
            if (rs.next()) {
              if (password.equals(rs.getString("password_hash"))) return true;  
            }
        }catch (SQLException exp) {
            exp.printStackTrace();
        } finally {
